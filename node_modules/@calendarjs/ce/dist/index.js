                ...
                <form :ref="self.form" data-readonly="{{self.readonly}}" class="lm-p20">
                    ...
                    <div class="lm-form-group lm-row">
                        <div class="lm-column lm-f1" style="margin-right: 8px;">
                            <Calendar input="auto" :footer="false" placeholder="When" :path="date" />
...
;(function (global, factory) {
    ...
    const Schedule = function(children, { onload, onchange, track }) {
        ...
        const setPointer = function() {
            ...
            // Get the position from the header to avoid issues when the component is not yet visible
            let header = self.el.querySelector('thead td[data-selected="true"]');
            if (header) {
                let reference = self.el.querySelector('[data-y="' + hour + '"]');
                if (reference) {
                    self.pointer.style.top = reference.offsetTop + 'px';
                    self.pointer.style.left = header.offsetLeft + 'px';
                    self.pointer.style.width = header.offsetWidth + 'px';
        ...
        const EventElement = function(children, { onload }) {
            ...
            return render => render`<div class="lm-schedule-item" id="{{this.guid}}" data-visible="{{self.visible}}" data-warning="{{self.warning}}" data-color="{{self.color}}" data-readonly="{{self.readonly}}" data-title="{{self.title}}" data-start="{{self.start}}" data-end="{{self.end}}" data-height="{{end-start}}"></div>`;
        }
            ...
            if (record) {
                ...
                if (JSON.stringify(newValue) !== JSON.stringify(oldValue)) {
                    ...
                    if (typeof (self.onchangeevent) == 'function') {
                        self.onchangeevent(self, newValue, oldValue);
                    }
        ...
        onchange((prop) => {
            if (prop === 'type' || prop === 'grid' || prop === 'weekly' || prop === 'value') {
                render();
            }
        });
    ...
    const Timeline = function (children, { onload, onchange, track }) {
        ...
        const updateResult = function () {
            ...
            for (let i = 0; i < result.length; i++) {
                result[i].day = getDate(toDate(result[i].date).toLocaleDateString('en-GB', { year: 'numeric', month: 'short', day: '2-digit' }));
            }
        ...
        return render => render`<div class="lm-timeline">
            ...
            <div :loop="self.result" data-message="{{self.message}}" data-mode="{{self.position}}" data-align="{{self.align}}" class="lm-timeline-data">
                <div class="lm-timeline-item" data-bullet="{{self.day}}">
                    <div class="lm-timeline-edit" :render="self.editable"><button type="button" class="lm-timeline-icon lm-ripple" onclick="${edition}" tabindex="0">edit</button></div>
